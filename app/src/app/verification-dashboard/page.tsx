...
export default function VerificationDashboard() {
  ...
  const transformDataset = async (dataset: Dataset): Promise<DashboardDataset> => {
    ...
    } else if (verificationInfo.timestamp > 0) {
    ...
    return {
      id: dataset.id.toString(), // Convert to string
      name: dataset.name,
      ...
      tags,
      rating: Math.min(5, Math.max(1, qualityScore / 20)) // Convert quality score to 1-5 rating
    };
  };
    ...
    const fetchData = async () => {
      try {
        ...
        const transformedDatasets = await Promise.all(
          blockchainDatasets.map(transformDataset)
        );
        ...
        const verifiedDatasets = transformedDatasets.filter(d => d.verificationStatus === 'verified').length;
        const pendingVerification = transformedDatasets.filter(d => d.verificationStatus === 'pending').length;
        const rejectedDatasets = transformedDatasets.filter(d => d.verificationStatus === 'rejected').length;
        const averageQualityScore = totalDatasets > 0 ? 
          transformedDatasets.reduce((sum, d) => sum + d.qualityScore, 0) / totalDatasets : 0;
        ...
        setStats({
          ...
          totalVerifiers: new Set(transformedDatasets.map(d => d.verifier).filter(Boolean)).size
        });
